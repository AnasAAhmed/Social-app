  ...
  const renderUser = (user: any, isFollower: boolean) => (
    <div key={user.id} className="bg-white dark:bg-slate-700 shadow-md rounded-lg p-4 w-full flex flex-col sm:flex-row items-center gap-4">
      ...
      <div className="flex items-center gap-3">
        <SmartLink href={`/profile/${user.username}`} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700">
          Profile
        </SmartLink>
      ...
      <Suspense fallback={<LoaderGif />}>
        ...
        <div className="flex flex-col gap-6 w-full lg:w-4/5">
          ...
          <div className="flex justify-center mb-4">
            <SmartLink href="?filter=followers" className={`mr-4 ${isFollowers ? 'text-blue-500' : 'text-gray-500'}`}>
              Followers {isFollowers && `(${totalCount})`}
            </SmartLink>
            <SmartLink href="?filter=followings" className={`${!isFollowers ? 'text-blue-500' : 'text-gray-500'}`}>
