...
import UserInfoCardForMob from '@/components/userInfo/UserInfoCardForMob';
import UserMediaCardForMob from '@/components/userMedia/UserMediaCardForMob';

const ProfilePage = async ({ searchParams, params }: { searchParams: Promise<{ page?: string, filter: string }>, params: Promise<{ username: string }> }) => {
    ...
    <div className='flex justify-end'>
      ...
      <div className="w-full md:w-[70%] xl:w-1/2 xl:mx-auto">
        <div className="flex flex-col gap-6">
          ...
          <div className='xl:hidden'> <UserMediaCardForMob userId={user.id} /></div>
          <div className='xl:hidden'> <UserInfoCardForMob userId={user.id} /></div>
          {/* <Suspense fallback={<LoaderGif />}> */}
            <Feed page={Number(page) || 1} username={user.username} />
          {/* </Suspense> */}
        </div>
      </div>
