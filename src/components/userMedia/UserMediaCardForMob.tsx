...
const UserMediaCardForMob = ({ userId }: { userId: string }) => {
    const isMobile = useMediaQuery('(max-width: 1280px)')

    const { data, isLoading, error, isError } = useUserMedia(userId, isMobile)

    if (!isMobile) return <Loader1 />
                    ...
                    data.map((post) => (
                        <div key={post.id} className="relative w-[27%] h-24">
                            {post.img?.endsWith(".mp4") ||
                                post.img?.endsWith(".webm") ||
                                post.img?.endsWith(".ogg") ? (
...
export default UserMediaCardForMob
