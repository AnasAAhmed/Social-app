    ...
    return (
        <div className='md:hidden' ref={dropdownRef}>
            ...
            <div onClick={() => setTimeout(() => toggleClose(), 140)} className={`fixed duration-500 left-0 ${open ? 'translate-x-[0%]' : '-translate-x-[150%]'} dark:text-gray-300 top-0 w-[87%] px-1 h-[110vh] pb-20 overflow-y-auto dark:bg-[#111] bg-white flex flex-col items-center justsify-center font-medium text-xl z-30`}>
                ...
                <SmartLink href={"/"} className='font-bold text-xl text-blue-600 my-4'>ANAS_SOCIAL</SmartLink>
                <div className='px-4 my-4' onClick={(e) => e.stopPropagation()}>
                    <Search />
                </div>
                <div className="flex flex-col gap-5 justify-center items-center">
                    <SmartLink className='dark:s' href={'/'}>Home</SmartLink>
                    <SmartLink href={'/friends'}>Friends</SmartLink>
                    <SmartLink href={'/friends/friend-requests'}>Friend Requests</SmartLink>
                    <SmartLink href={'/friends/suggestions'}>Suggestions</SmartLink>
                    <SmartLink href={'/stories'}>Stories</SmartLink>
                    <SmartLink href={'/settings#'}>Settings</SmartLink>
                    <SmartLink href={'/settings#/security'}>Security</SmartLink>
                    <SmartLink href={`/profile/${session?.user?.name}`}>My Profile</SmartLink>
                </div>
            </div>
            {/* )}  */}
        </div>
