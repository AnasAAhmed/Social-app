...
import { calculateTimeDifference, extractUrl } from '@/lib/utils'
...
const Post = ({ post, userId }: { post: feedPostsType, userId: string }) => {
    ...
    const url = extractUrl(post.desc);
        ...
        <div className="flex flex-col dark:bg-[#111] bg-white sm:rounded-md py-6 gap-4 border-b dark:border-slate-700">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 mx-4">
                    ...
                    <SmartLink className='flex flex-col' href={`/profile/${post.user.username}`}>
            ...
            {/* desc */}
            <div className='mx-4 text-xs md:text-sm dark:text-gray-300'>
                <Truncate postBy={post.user.username} postId={post.id} desc={post.desc} numOfChar={100} />
            </div>
                            ...
                            <div className="w-full min-h-96 relative ">
                                {url && post.img ?
                                    <img
                                        ...
                                        alt="Uploaded Image"
                                        className="absolute inset-0 w-full h-full object-cover sm:rounded-md"
                                    />
